    ...
    int32_t b_w, int32_t b_h, int32_t w, int32_t h)
{
    ...
    if (left) {
        ...
        for (int32_t i = 0; i < blk_fill_itt; i++) {
            ...
            if (right) eb_memcpy((void*)(dst_temp + left), (void*)ref_temp, right * sizeof(uint16_t));
            dst_temp += dst_stride;
            ++y0;
    ...
    if (right) {
        ...
        for (int32_t i = 0; i < blk_fill_itt; i++) {
            if (copy) eb_memcpy((void*)dst_temp, (void*)(ref_temp + x0), copy * sizeof(uint16_t));
    ...
    if (top) {
        ...
        for (int32_t i = 0; i < top; i++) {
            eb_memcpy((void*)dst, (void*)dst_temp, b_w * sizeof(uint16_t));
            dst += dst_stride;
        }
    }
    ...
    do {
        ...
        if (copy) eb_memcpy((void*)(dst + left), (void*)(ref_row + x + left), copy * sizeof(uint16_t));
    ...
    int32_t b_h, int32_t w, int32_t h)
{
    ...
    if (left) {
        ...
        for (int32_t i = 0; i < blk_fill_itt; i++) {
            ...
            if (right) eb_memcpy((void*)(dst_temp + left, ref_temp, right);
            dst_temp += dst_stride;
            ++y0;
            if (y0 > 0 && y0 < h) ref_temp += src_stride;
        }
    ...
    if (right) {
        ...
        for (int32_t i = 0; i < blk_fill_itt; i++) {
            if (copy) eb_memcpy((void*)dst_temp, (void*)ref_temp + x0, copy);
    ...
    if (top) {
        dst_temp = dst + (top * dst_stride);
        for (int32_t i = 0; i < top; i++) {
            eb_memcpy((void*)dst, (void*)dst_temp, b_w);
            dst += dst_stride;
        }
    }
    ...
    do {
        ...
        if (copy) eb_memcpy((void*)(dst + left), (void*)(ref_row + x + left), copy);

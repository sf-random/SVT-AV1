...
static void extend_frame_lowbd(uint8_t *data, int32_t width, int32_t height, int32_t stride,
    ...
    for (i = -border_vert; i < 0; ++i) eb_memcpy((void*)(data_p + i * stride),(void*)data_p, width + 2 * border_horz);
    for (i = height; i < height + border_vert; ++i) {
        eb_memcpy((void*)(data_p + i * stride),(void*)(data_p + (height - 1) * stride), width + 2 * border_horz);
    }
}
...
static void extend_frame_highbd(uint16_t *data, int32_t width, int32_t height, int32_t stride,
    ...
    for (i = -border_vert; i < 0; ++i) {
        eb_memcpy((void*)(data_p + i * stride), (void*)data_p, (width + 2 * border_horz) * sizeof(uint16_t));
    }
    for (i = height; i < height + border_vert; ++i) {
        eb_memcpy((void*)(data_p + i * stride),
               (void*)(data_p + (height - 1) * stride),
...
static void copy_tile_lowbd(int32_t width, int32_t height, const uint8_t *src, int32_t src_stride,
    ...
    for (int32_t i = 0; i < height; ++i) eb_memcpy((void*)(dst + i * dst_stride), (void*)(src + i * src_stride), width);
}
...
static void copy_tile_highbd(int32_t width, int32_t height, const uint16_t *src, int32_t src_stride,
    ...
    for (int32_t i = 0; i < height; ++i)
        eb_memcpy((void*)(dst + i * dst_stride), (void*)(src + i * src_stride), width * sizeof(*dst));
}
    ...
    if (!opt) {
        if (copy_above) {
            ...
            for (int32_t i = -RESTORATION_BORDER; i < 0; ++i) {
                ...
                eb_memcpy(rlbs->tmp_save_above[i + RESTORATION_BORDER],
                       ...
                       line_size);
                eb_memcpy(REAL_PTR(use_highbd, dst8), (void*)buf, line_size);
            }
        }
        ...
        if (copy_below) {
            ...
            for (int32_t i = 0; i < RESTORATION_BORDER; ++i) {
                ...
                eb_memcpy((void*)rlbs->tmp_save_below[i], REAL_PTR(use_highbd, dst8), line_size);
                eb_memcpy(REAL_PTR(use_highbd, dst8), (void*)src, line_size);
            }
        }
    } else {
        if (copy_above) {
            ...
            // Save old pixels, then replace with data from stripe_boundary_above
            eb_memcpy((void*)(rlbs->tmp_save_above[0]), REAL_PTR(use_highbd, dst8), line_size);
        ...
        if (copy_below) {
            ...
            // Save old pixels, then replace with data from stripe_boundary_below
            eb_memcpy((void*)(rlbs->tmp_save_below[2]), REAL_PTR(use_highbd, dst8), line_size);
            eb_memcpy(REAL_PTR(use_highbd, dst8),
    ...
    if (!opt) {
        if (copy_above) {
            ...
            for (int32_t i = -RESTORATION_BORDER; i < 0; ++i) {
                uint8_t *dst8 = data8_tl + i * data_stride;
                eb_memcpy(REAL_PTR(use_highbd, dst8),
                        (void*)rlbs->tmp_save_above[i + RESTORATION_BORDER],
                       line_size);
            }
        }
        ...
        if (copy_below) {
            ...
            for (int32_t i = 0; i < RESTORATION_BORDER; ++i) {
                ...
                uint8_t *dst8 = data8_bl + i * data_stride;
                eb_memcpy(REAL_PTR(use_highbd, dst8), (void*)rlbs->tmp_save_below[i], line_size);
            }
        }
    } else {
        if (copy_above) {
            ...
            uint8_t *dst8 = data8_tl + (-RESTORATION_BORDER) * data_stride;
            eb_memcpy(REAL_PTR(use_highbd, dst8), (void*)rlbs->tmp_save_above[0], line_size);
        }
        ...
        if (copy_below) {
            ...
            if (stripe_bottom + 2 < limits->v_end + RESTORATION_BORDER) {
                uint8_t *dst8 = data8_bl + 2 * data_stride;
                eb_memcpy(REAL_PTR(use_highbd, dst8), (void*)rlbs->tmp_save_below[2], line_size);
            }
        }
    }
}
...
void save_deblock_boundary_lines(uint8_t *src_buf, int32_t src_stride, int32_t src_width,
    ...
    if (!av1_superres_unscaled(&cm->frm_size)) {
        ...
        for (int32_t i = 0; i < lines_to_save; i++) {
            eb_memcpy((void*)(bdry_rows + i * bdry_stride), (void*)(src_rows + i * src_stride), line_bytes);
        }
    }
...
void save_cdef_boundary_lines(uint8_t *src_buf, int32_t src_stride, int32_t src_width,
    ...
    for (int32_t i = 0; i < RESTORATION_CTX_VERT; i++) {
        ...
        eb_memcpy((void*)(bdry_rows + i * bdry_stride), (void*)src_rows, line_bytes);
    }
    extend_lines(bdry_rows,
                 upscaled_width,
                 RESTORATION_CTX_VERT,

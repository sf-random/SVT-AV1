...
void eb_av1_filter_intra_predictor_sse4_1(uint8_t *dst, ptrdiff_t stride, TxSize tx_size,
    ...
    eb_memcpy((void*)buffer[0], (void*)(&above[-1]), (bw + 1) * sizeof(uint8_t));
    ...
    for (r = 1; r < bh + 1; r += 2) {
        for (c = 1; c < bw + 1; c += 4) {
            ...
            eb_memcpy((void*)p, (void*)&buffer[r - 1][c - 1], 5 * sizeof(uint8_t));
            p[5]                       = buffer[r][c - 1];
            p[6]                       = buffer[r + 1][c - 1];
            p[7]                       = 0;
    ...
    for (r = 0; r < bh; ++r) {
        eb_memcpy((void*)dst, (void*)&buffer[r + 1][1], bw * sizeof(uint8_t));
        dst += stride;
    }
}

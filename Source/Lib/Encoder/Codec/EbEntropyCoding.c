...
int32_t av1_write_coeffs_txb_1d(PictureParentControlSet *parent_pcs_ptr,
    ...
    if (component_type == COMPONENT_LUMA) {
        av1_write_tx_type(
            ...
            uint32_t total_samples = (width*height);
            uint32_t count_non_zero_coeffs   = blk_ptr->txb_array[txb_index].nz_coef_count[0];
            uint8_t band_idx = 0;
            ...
            //uint64_t    txt_stats[2/*inter-intra*/][10/*coef*/][TX_SIZES_ALL][TX_TYPES];
            context_ptr->txt_stats[is_inter][band_idx][tx_size][tx_type]++;

            // by size
            context_ptr->tx_type_size[tx_size][tx_type]++;
            ///////////


            // by coef

            context_ptr->tx_type_coef[band_idx][tx_type]++;
            ///////////

            // by type
       
            context_ptr->tx_type_ii_type[is_inter][tx_type]++;
            ///////////
...
EB_EXTERN EbErrorType write_sb(EntropyCodingContext *context_ptr, SuperBlock *tb_ptr,
...
#if ENC_STATS
    memset(context_ptr->txt_stats,0,sizeof(uint64_t)*TX_SIZES_ALL*TX_TYPES*2*2);
    memset(context_ptr->tx_type_size,0,sizeof(uint64_t) * TX_SIZES_ALL*TX_TYPES);
    memset(context_ptr->tx_type_coef,0,sizeof(uint64_t) * 10*TX_TYPES);
    memset(scs_ptr->tx_type_ii_type,0,sizeof(uint64_t)*2 * TX_TYPES);
    ...
    for (TxSize txs = 0 ; txs < TX_SIZES_ALL ; ++txs)
        for (TxType txt = 0 ; txt < TX_TYPES ; ++txt)
            scs_ptr->tx_type_size[txs][txt] += context_ptr->tx_type_size[txs][txt];
    ...
    for (int ii_type = 0 ; ii_type < 2 ; ++ii_type)
        for (TxType txt = 0 ; txt < TX_TYPES ; ++txt)
            scs_ptr->tx_type_ii_type[ii_type][txt] += context_ptr->tx_type_ii_type[ii_type][txt];

    for (int type = 0 ; type < 2 ; ++ type)
        for (int band = 0 ; band < 2 ; ++ band)
            for (int txsize = 0 ; txsize < TX_SIZES_ALL ; ++ txsize)
                for (TxType txtype = 0 ; txtype < TX_TYPES ; ++txtype)
                    scs_ptr->txt_stats[type][band][txsize][txtype] +=
                context_ptr->txt_stats[type][band][txsize][txtype];
